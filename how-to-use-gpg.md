...
공개키 암호화 방식에 관한 내용은 [공개키 암호화 방식이란?](/posts/what-is-public-key-cryptography/)을 참조한다. 
...
타인에게 공개할 공개키를 공유 하기 위해서는 키를 내보야 한다. `--export` 옵션을 사용하여 키를 내보낸다. 
...
다른 사람들이 공개키를 검증 하는 것을 허용 하려면 공개키의 지문(fingerprint)도 같이 공유 한다. 
...
`doc.txt` 을 암호화하고 개인키로 서명한다. 서명에 사용될 키를 지정 해주지 않으면 기본 개인 키가 사용된다. 
...
* `--sign` 옵션으로 signature를 생성한다. 자신의 개인키로 서명을 한다. 복호화를 위해서는 서명에 사용한 개인키에 대응되는 공개키카 설치 되어야 한다. 
...
잠시 용어를 정리할 필요가 있다. GPG(GnuPG)에서는 *암호화(Encryption)*와 *서명(Signing)*이라는 두가지 용어를 사용하는 데 *암호화* 는 공개키를 이용하여 암호화 하는 방법이고, *서명(Signing)* 은 개인키를 이용하여 암호화 하는 방식이다. 

공개키를 이용하여 암호화 하였을 경우 개인키를 가진 사람만이 정보를 열람 할 수 있으므로 *암호화(Encryption)* 라고 하지만 개인키를 이용하여 암호화 하였을 경우 공개키를 가진 모든 사람(공개키가 키 서버에 공개 되어 있다면 잠재적으로 누구든지)이 해당 정보를 열람 할 수 있으므로 암호라고 하지는 않는다. 하지만 개인키를 통하여 암호화 되었기 때문에 암호화에 사용된 개인키에 대응되는 공개키를 가진 사람은 이 공개키로 복호화 된다면 이는 해당 정보의 출처가 (이 정보에 *서명(Signing)* 한 개인키에 대응되는)공개키를 공개한 사람 이라는 것과 데이터가 (작성자가 서명한 이후)변조 되않았다는 것을 증명하기 때문에 *서명(Signing)* 이라고 한다. 


> 따라서 `--sign`옵션을 주면 *받는사람(Recipient)* 의 공개키로 *암호화(Encryption)* 하고 이 암호화된 정보의 자신의 개인키로 ~~*다시한번 암호화(Encryption)*~~ *서명(Signing)* 하는 것이다. 
...
> 서명이된 암호화 파일의 경우 서명한 사람의 공개키가 설치 되어 있어야 정상적으로 복호화가 된다. 
...
이메일 주소를 기반으로 공개키를 등록하고 받게 해주는 공개 키 서버가 있다. 
...
> 키를 등록하면 키 생성시 입력한 메일 주소로 확인 메일이 온다. 설명에 따라 인증을 수행한다. 
...
위 명령을 수행하면 `john@acme.com` 와 관련된 공개키 리스트가 나오고 키를 선택 하라는 프롬프트가 뜬다. 
